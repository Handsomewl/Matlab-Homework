...
info_rebuild='';
for idx=1:16
     col=ceil(idx/8);row=idx-(col-1)*8;
     if mod(Hide(row,col),2)
         valid_len=strcat(valid_len,'1');
     else
         valid_len=strcat(valid_len,'0');
     end
end
valid_len=bin2dec(valid_len);
bin_code='';
ASCII=zeros(1,valid_len/8);
for idx=1:valid_len
     col=ceil(idx/8);row=idx-(col-1)*8;col=col+2;
     if mod(Hide(row,col),2)
         bin_code=strcat(bin_code,'1');
     else
         bin_code=strcat(bin_code,'0');
     end
end
%}
...
for idx=1:valid_len/8
    ASCII(1,idx)=bin2dec(bin_code((idx-1)*8+1:idx*8));
end
info=char(ASCII);
...
% PSNR
error_matrix=abs(hall_gray-Hallgray);
...
%{
figure;
subplot(1,2,1);imshow(hall_gray);title("编码前");
