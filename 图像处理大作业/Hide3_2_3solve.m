...
for idx=1:16
    END=64;
    while Hide(END,idx)==0
        END=END-1;
    end
    valid_len=strcat(valid_len,dec2bin((Hide(END,idx)+1)/2));
end
valid_len=bin2dec(valid_len);
for idx=1:min(valid_len,len)
    END=64;
    while Hide(END,idx+16)==0
        END=END-1;
    end
    bin=strcat(bin,dec2bin((Hide(END,idx+16)+1)/2));
end
ASCII=zeros(1,valid_len/8);
for idx=1:valid_len/8
     ASCII(1,idx)=bin2dec(bin((idx-1)*8+1:idx*8));
end
info=char(ASCII);
